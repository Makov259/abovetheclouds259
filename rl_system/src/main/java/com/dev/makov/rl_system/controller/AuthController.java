...
public class AuthController {
    ...
    public String registerParent(Model model) {
        model.addAttribute("user", new User());
        List<User> students = userService.findAllStudents(); // Fetch list of students
    ...
    public String registerParentUser(@ModelAttribute("user") User user,
        ...
        if (studentId != null) {
            userService.registerParentWithStudent(user, studentId); // Register parent with student
        } else {
            userService.registerParent(user); // Register parent without student association
        }
        return "redirect:/login";
    }
}
